...
export default function Database() {
  ...
  const [projectData, setProjectData] = useState<DatabaseProject | null>(null)
  ...
  const handleSearch = (e: React.FormEvent) => {
    ...
    const found = content.databasePage.projects.find(
      p => p.code.toLowerCase().trim() === searchCode.toLowerCase().trim()
    )
    ...
    <div className="min-h-screen bg-reform-red px-4 sm:px-6 md:px-8 py-6 md:py-8">
      ...
      <main className="w-full max-w-[1860px] mx-auto animate-slide-up">
        <div className="mb-6 md:mb-8">
          ...
          <p className="text-[14px] md:text-[16px] text-reform-black/80">{db.subtitle}</p>
        </div>
      </main>
      ...
      <main className="w-full max-w-[1860px] mx-auto mb-16">
        <form onSubmit={handleSearch} className="w-full max-w-[800px] mx-auto flex gap-0">
          ...
          />
          <button type="submit" className="bg-reform-black hover:bg-reform-black/90 transition-colors px-6 md:px-8 py-4 md:py-5 flex items-center justify-center" aria-label="Search">
        ...
        <main className="w-full max-w-[1860px] mx-auto px-4 sm:px-6 md:px-8 pb-24">
          {showResults && projectData ? (
            <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,2fr)_minmax(0,3fr)] gap-8 md:gap-12 animate-slide-up">
              <div className="bg-reform-black/10 border border-reform-black/20 rounded-none p-6 md:p-8">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <h2 className="text-[20px] md:text-[24px] font-bold text-reform-black">{db.projectInfoTitle}</h2>
                </div>
                ...
                <div className="space-y-4 text-reform-black">
                  <div>
                    <p className="text-[14px] md:text-[16px] text-reform-black/60 mb-1 tracking-wide">{projectData.status}</p>
              ...
              <div className="bg-reform-black/10 border border-reform-black/20 rounded-none p-6 md:p-8">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <h2 className="text-[20px] md:text-[24px] font-bold text-reform-black">{db.projectFilesTitle}</h2>
                </div>
                ...
                <div className="space-y-4">
                  {projectData.files.map((file, index) => (
                    <div key={index} className="bg-reform-black border border-reform-black/20 rounded-none p-4 md:p-6 flex items-center gap-4 sm:gap-5 md:gap-6">
                      <div className="bg-[#1a1a1a] text-white text-[14px] sm:text-[16px] md:text-[18px] font-bold px-3 sm:px-4 md:px-5 py-6 sm:py-7 md:py-8 rounded-none shrink-0 flex items-center justify-center min-w-[60px] sm:min-w-[70px] md:min-w-[80px]">
                        {file.type}
                      </div>
                      ...
                      <a href={file.downloadUrl} download className="flex items-center gap-2 text-[13px] sm:text-[14px] md:text-[16px] font-semibold text-reform-red hover:text-white transition-colors border-b border-reform-red hover:border-white pb-1 shrink-0">
            ...
            <div className="text-center py-16 animate-slide-up">
              <p className="text-[18px] md:text-[24px] text-reform-black/80">{db.notFoundTitle} &quot;{searchCode}&quot;</p>
              <p className="text-[14px] md:text-[16px] text-reform-black/60 mt-2">{db.notFoundSubtitle}</p>
            </div>
          ) : null}
        </main>
      )}
