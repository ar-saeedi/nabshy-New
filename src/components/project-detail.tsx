...
export default function ProjectDetail({ projectId }: ProjectDetailProps) {
  ...
  const totalSlides = project.carouselSlides?.length || 0
  ...
  const nextProjectsData = project.nextProjects?.map(id => content.projects.find(p => p.id === id)).filter(Boolean) || []
    ...
    <div className="bg-black text-white min-h-screen">
      <section className="relative min-h-screen w-full overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col min-h-screen px-4 sm:px-6 md:px-8 py-6 sm:py-8">
          <header className="w-full max-w-[1860px] mx-auto flex justify-between items-start gap-4 mb-8 md:mb-12 animate-slide-up">
            <Link href="/" className="text-[20px] xs:text-[22px] sm:text-[24px] md:text-[28px] lg:text-[32px] font-bold leading-none text-white z-50">
              NABSHY
            </Link>
            ...
            <button onClick={() => setMobileMenuOpen(!mobileMenuOpen)} className="lg:hidden flex flex-col gap-1.5 p-2 z-50 relative" aria-label="Toggle menu">
              ...
              <div className="lg:hidden fixed inset-0 bg-black/40 z-40 backdrop-blur-sm" onClick={() => setMobileMenuOpen(false)}>
                <div className="absolute right-0 top-0 h-full w-[70%] max-w-[280px] bg-black shadow-2xl" onClick={(e) => e.stopPropagation()}>
                  <nav className="flex flex-col gap-8 items-start p-8 mt-20">
                    <Link href="/projects" onClick={() => setMobileMenuOpen(false)} className="text-[20px] font-bold text-white hover:opacity-70 transition-opacity">PROJECTS</Link>
                    <Link href="/database" onClick={() => setMobileMenuOpen(false)} className="text-[20px] font-bold text-white hover:opacity-70 transition-opacity">DATABASE</Link>
                    <Link href="/studio" onClick={() => setMobileMenuOpen(false)} className="text-[20px] font-bold text-white hover:opacity-70 transition-opacity">STUDIO</Link>
                    <Link href="/contact" onClick={() => setMobileMenuOpen(false)} className="text-[20px] font-bold text-white hover:opacity-70 transition-opacity">CONTACT</Link>
                  </nav>
                </div>
              </div>
            )}
      ...
      <section className="w-full bg-black px-4 sm:px-6 md:px-8 pt-16 pb-24 md:pt-24 md:pb-36">
        <div className="w-full max-w-[1860px] mx-auto grid grid-cols-1 lg:grid-cols-[minmax(0,2fr)_minmax(0,1.6fr)] gap-12">
          <div>
            <h2 className="text-[24px] sm:text-[28px] md:text-[32px] font-bold mb-6">{project.title}</h2>
          ...
          <div className="text-[13px] sm:text-[14px] md:text-[15px]">
            ...
            <div className="grid grid-cols-[1.6fr_1fr_0.7fr] gap-x-8 text-white">
              ...
              <div className="pt-3">{project.client}</div>
              <div className="pt-3">{project.year}</div>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="w-full bg-black px-4 sm:px-6 md:px-8 pt-16 pb-10 md:pt-24 md:pb-12">
        <div className="w-full max-w-[1860px] mx-auto">
          <h2 className="text-[24px] sm:text-[32px] md:text-[40px] lg:text-[48px] font-bold leading-tight max-w-[720px]">
            {project.sectionTitle}
          </h2>
        </div>
      </section>

      {totalSlides > 0 && project.carouselSlides[slideIndex] && (
        ...
        <section className="w-full bg-black px-4 sm:px-6 md:px-8 pt-12 pb-24">
          ...
          <div className="w-full max-w-[1860px] mx-auto">
            <div className="flex items-center justify-between mb-8">
              <h2 className="text-[24px] sm:text-[28px] md:text-[32px] lg:text-[40px] font-bold">NEXT PROJECTS</h2>
              <Link href="/projects" className="text-[13px] sm:text-[14px] md:text-[15px] font-medium tracking-wide border-b border-white pb-1 hover:opacity-80 transition-opacity">VIEW ALL PROJECTS</Link>
            </div>
                ...
                <Link key={nextProject!.id} href={nextProject!.href} className="group flex flex-col gap-4">
                  ...
                  <div className="flex flex-wrap gap-3 mt-3">
                    {nextProject!.features.map((feature) => (
                      <span key={feature} className="px-4 py-1 border border-white rounded-none text-[12px] sm:text-[13px] md:text-[14px]">{feature}</span>
                    ))}
                  </div>
                  <div className="mt-2 flex flex-col gap-1">
                    <h3 className="text-[18px] sm:text-[20px] md:text-[22px] font-bold">{nextProject!.title}</h3>
                    <p className="text-[14px] sm:text-[15px] md:text-[16px] text-white/85">{nextProject!.subtitle}</p>
                  </div>
                </Link>
              ))}
            </div>
          ...
          <div className="flex flex-col md:flex-row justify-between items-start gap-8 md:gap-12">
            <div className="flex flex-col xs:flex-row gap-8 xs:gap-12 md:gap-16">
              <div className="flex flex-col gap-4 md:gap-6">
                <h3 className="text-[12px] sm:text-[13px] md:text-[14px] font-extralight tracking-wider text-white/80">NAVIGATE</h3>
                <nav className="flex flex-col gap-2 md:gap-3 text-[14px] sm:text-[16px] md:text-[18px] font-extralight">
                  {footer.navigation.map((link, i) => (
                    <Link key={i} href={link.href} className="hover:opacity-70 transition-opacity">{link.label}</Link>
                  ))}
                </nav>
              </div>

              <div className="flex flex-col gap-4 md:gap-6">
                <h3 className="text-[12px] sm:text-[13px] md:text-[14px] font-extralight tracking-wider text-white/80">SOCIAL</h3>
                <nav className="flex flex-col gap-2 md:gap-3 text-[14px] sm:text-[16px] md:text-[18px] font-extralight">
                  {footer.social.map((link, i) => (
                    <a key={i} href={link.href} target="_blank" rel="noopener noreferrer" className="hover:opacity-70 transition-opacity">{link.label}</a>
                  ))}
                </nav>
              </div>
            </div>
            ...
            <div className="flex flex-col items-start md:items-end gap-4 md:gap-6 w-full md:w-auto">
              ...
              <Link href="/contact" className="group relative flex items-center pb-1.5">
                <span className="text-[14px] sm:text-[16px] md:text-[18px] lg:text-[20px] font-semibold leading-[22px] tracking-wide">GET IN TOUCH</span>
                <div className="absolute bottom-0 left-0 right-0 h-[1px] bg-white" />
              </Link>
            </div>
          </div>

          <div ref={footerRef} className={`flex items-center justify-center transition-all duration-[900ms] ease-[cubic-bezier(0.22,0.61,0.36,1)] ${footerVisible ? 'animate-slide-up' : 'opacity-0 translate-y-16'}`}>
            <h2 className="text-[clamp(80px,15vw,260px)] font-bold leading-[0.9] tracking-tight text-white">{footer.brandName}</h2>
          </div>

          <div className="flex flex-col xs:flex-row justify-between items-center gap-3 xs:gap-4 text-center xs:text-left">
            <div className="text-[11px] xs:text-[12px] sm:text-[13px] md:text-[14px] lg:text-[16px] font-extralight text-white/70">{footer.copyright}</div>
          </div>
        </div>
      </section>
    </div>
